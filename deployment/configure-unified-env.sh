...
if [[ "${DEPLOY_MODE}" == "production" ]]; then
    ...
    export BILLIONMAIL_URL="https://${MAIL_DOMAIN}:8080/billion"
    export BILLIONMAIL_WEB="https://${MAIL_DOMAIN}:8080/billion"
    export BILLIONMAIL_API="https://${MAIL_DOMAIN}:8080/api/v1"
    export MEILISEARCH_URL="https://${DOMAIN}:7700"
else
    # 开发环境使用HTTP
    export FRONTEND_URL="http://${DOMAIN}"
    export BACKEND_URL="http://${DOMAIN}:1337"
    export ADMIN_URL="http://${DOMAIN}:1337/admin"
    export BILLIONMAIL_URL="http://${DOMAIN}:8080/billion"
    export BILLIONMAIL_WEB="http://${DOMAIN}:8080/billion"
    export BILLIONMAIL_API="http://${DOMAIN}:8080/api/v1"
    export MEILISEARCH_URL="http://${DOMAIN}:7700"
fi

...
show_access_info() {
    ...
    echo -e "${GREEN}   Web邮箱: https://$DOMAIN/webmail${NC}"
    echo -e "${GREEN}   API文档: https://$DOMAIN/documentation${NC}"
    echo ""
    echo -e "${BLUE}💡 本地开发地址：${NC}"
    echo -e "${YELLOW}   前端: http://localhost:3000${NC}"
    echo -e "${YELLOW}   后端: http://localhost:1337${NC}"
    echo -e "${YELLOW}   搜索: http://localhost:7700${NC}"
    echo -e "${YELLOW}   邮件: http://localhost:8080${NC}"
    echo ""
    echo -e "${BLUE}🔧 管理命令：${NC}"

...
execute_deployment() {
    ...
    # 邮件系统已集成到Strapi
    log_info "邮件营销系统已集成到Strapi后台，无需单独部署"
...
verify_deployment() {
    ...
    # 邮件系统已集成到Strapi
    log_info "邮件营销系统已集成到Strapi，无需额外检查"
...
show_deployment_result() {
    ...
    echo ""
    echo -e "${CYAN}📧 邮件系统:${NC}"
    echo "   管理界面: https://$DEPLOY_DOMAIN/admin (Strapi后台)"
    echo "   功能说明: 邮件营销系统已集成到Strapi"
    echo ""
    echo -e "${CYAN}💡 管理命令:${NC}"

...
graph TB
    subgraph "🎯 核心业务层"
        A[Next.js 14 前端 :80]
        B[Strapi 5.20.0 后端 :1337]
        C[PostgreSQL 主数据库 :5432]
        D[Redis 缓存 :6379]
        E[MeiliSearch 搜索 :7700]
    end
    
    subgraph "📧 邮件营销层"
        F[BillionMail Core :8080]
        G[PostgreSQL 邮件库 :5433]
        H[Redis 邮件缓存 :6380]
        I[rspamd 反垃圾 :11334]
        J[postfix SMTP :25/587]
        K[dovecot IMAP :143/993]
    end
...
| 3️⃣ | **配置生成** | 读取 `deploy.conf` 生成所有环境变量 | ~10秒 |
| 4️⃣ | **数据恢复** | 自动恢复演示数据(文章、用户等) | ~30秒 |
| 5️⃣ | **服务启动** | 按序启动: 数据库→搜索→邮件→后端→前端 | ~60秒 |
...
BILLIONMAIL_PASSWORD=email_pass    # 邮件系统密码

# 📦 自动化开关
AUTO_RESTORE_BACKUP=true          # 是否自动恢复数据
AUTO_DEPLOY_SEARCH=true           # 是否自动部署搜索
AUTO_DEPLOY_EMAIL=true            # 是否自动部署邮件
...
- **双库架构**: Strapi主业务库 + BillionMail邮件库
...
# 📧 邮件管理  
./scripts.sh billionmail check     # 检查邮件系统
./scripts.sh billionmail logs      # 查看邮件日志
./scripts.sh billionmail restart   # 重启邮件服务
```
...
curl http://localhost:8080/api/health         # 邮件健康检查
...
### ❓ **邮件系统无法启动**
```bash
# 问题: BillionMail启动失败或重启循环
# 解决方案:
1. 检查邮件状态: ./scripts.sh billionmail check
2. 查看详细日志: ./scripts.sh billionmail logs
3. 清理旧数据: sudo rm -rf BillionMail/rspamd-data
4. 重新部署: ./scripts.sh billionmail restart
```

...
AI变现之路项目采用**智能简化备份策略**，只备份核心数据，让系统自动重建缓存和索引，既保证数据完整性又提升备份效率。

### ✅ **备份内容**

| 类型 | 内容 | 说明 |
|------|------|------|
| **🗄️ PostgreSQL数据库** | Strapi主业务数据库 | 包含文章、用户、分类等核心数据 |
| **📧 BillionMail数据库** | 邮件系统数据库 | 包含邮件列表、营销数据 |
| **⚙️ 配置文件** | deploy.conf, .env, env_init | 系统配置和环境变量 |
| **📁 上传文件** | 媒体文件、附件 | 用户上传的图片、文档等 |
| **📦 项目文件** | package.json | 依赖管理配置 |
...
| **MeiliSearch索引** | 全文搜索索引 | 首次访问时自动同步 |
...
│   ├── databases/                       # 数据库文件
│   │   ├── strapi_database.sql          # Strapi数据库
│   │   └── billionmail_database.sql     # BillionMail数据库
│   ├── config/                          # 配置文件
...
备份内容:
- Strapi数据库: ✅ 已备份 (724K)
- BillionMail数据库: ✅ 已备份 (100K)
- 配置文件: ✅ 已备份
- Redis缓存: ⚠️ 跳过 (会自动重建)
- MeiliSearch索引: ⚠️ 跳过 (会自动重建)
...
# 📋 备份类型说明:
# - 数据库备份: 仅备份PostgreSQL数据库 (Strapi + BillionMail)
...
├── current_strapi.sql      # 当前Strapi数据
└── current_billionmail.sql # 当前BillionMail数据
```
